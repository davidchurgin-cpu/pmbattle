...
export interface Snapshot { events: Event[]; parentOrders: ParentOrder[]; orders: Order[]; positions: Position[]; settlements: Settlement[]; fills: Fill[]; health: Health; bankroll: Money; atRisk: Money; settings: Settings }
export interface AccountSnapshot { parentOrders: ParentOrder[]; orders: Order[]; positions: Position[]; settlements: Settlement[]; fills: Fill[]; bankroll: Money; atRisk: Money }
