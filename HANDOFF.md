...
- Known, not yet fixed: saving sport preferences restarts the exchange loop, which cancels an in-progress account sync (Available shows $0 until the next 30-second refresh) and has produced `database is locked (SQLITE_BUSY)` in save schedule. See "Next milestone".
...
Housekeeping still open: retention for completed parents and audit rows. Also open, lower priority: make preference saves not cancel the in-flight account reconciliation (or re-run it immediately after the restart) and remove the SQLite write contention between the schedule save and the exchange restart.
